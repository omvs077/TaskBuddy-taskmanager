...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    {
        if (bd->pd3dDevice->CreateVertexShader(g_ImGuiVertexShaderBytes, sizeof(g_ImGuiVertexShaderBytes), nullptr, &bd->pVertexShader) != S_OK)
            return false;
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, g_ImGuiVertexShaderBytes, sizeof(g_ImGuiVertexShaderBytes), &bd->pInputLayout) != S_OK)
            return false;
    ...
    {
        if (bd->pd3dDevice->CreatePixelShader(g_ImGuiPixelShaderBytes, sizeof(g_ImGuiPixelShaderBytes), nullptr, &bd->pPixelShader) != S_OK)
            return false;
    }
